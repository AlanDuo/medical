...
public class ImgUploadUtil {
    ...
    public static boolean uploadFile(String imgBase64,HttpServletRequest request){
        ...
        Long time=System.currentTimeMillis();
        String originalFileName=time.toString().substring(time.toString().length()-8,time.toString().length());
        originalFileName=originalFileName.concat(".png");
        url=basePath+"/static/image/"+originalFileName;
        try{
            outputStream=new FileOutputStream(REAL_FILE+originalFileName);
            byte[] imageByteArray=decoder.decodeBuffer(imgBase64);
